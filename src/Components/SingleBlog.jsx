...
const SingleBlog = ({blog,handleBookmark,markAsRead}) => {
    const {_id,image, authorName, authorImage,publishDate,readTime,blogTitle} = blog;
        ...
        <div>
            ...
            <div className='flex justify-between items-center my-5'>
              ...
              <div className='flex justify-center items-center gap-2'>
                <p className="text-md text-gray-700">{readTime} minutes</p>
                <p className="text-sm">
                  <i onClick={() => {setIsBookmarked(true); handleBookmark(blogTitle)}} 
                    className={isBookmarked ? "fa-solid fa-bookmark cursor-pointer" : "fa-regular fa-bookmark cursor-pointer"}
                    ></i>
                </p>
              </div>
            </div>
            <h2 className="text-3xl font-bold mb-4 leading-snug">{blogTitle}</h2>
            <p className="text-sm text-gray-600">#beginners #programming</p>
            <span onClick={() => {markAsRead(readTime)}}
